...
export function PurchaseModal({ isOpen, onClose, serviceType, refreshProfile }: PurchaseModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Open receipt with transaction data
      onClose()
      setTimeout(() => {
        // We'll use a global state or a callback to open the receipt
        // For now, I'll update DashboardPage to handle this
              ...
              <div className="p-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <Button type="submit" disabled={loading} className="mt-4 h-14 gap-3 bg-brand-cyan hover:bg-brand-cyan/90 text-brand-midnight font-black shadow-[0_0_30px_rgba(0,212,255,0.2)]">
                    {loading
                      ? <div className="w-5 h-5 border-2 border-brand-midnight/30 border-t-brand-midnight rounded-full animate-spin" />
                      : <><span>Confirm Purchase</span><ArrowRight size={16} /></>
                    }
                  </Button>
                </form>
